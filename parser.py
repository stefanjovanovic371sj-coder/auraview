...
Metrics = {
    ...
    "HIGH": {"aliases": ["high", "tar", "above range", "visoko", "високо"], "unit": "%", "type": "RANGE_COMPONENT"},
    "VERY_HIGH": {"aliases": ["very high", "very-high", "vrlo visoko", "веома високо", "veoma visoko"], "unit": "%", "type": "RANGE_COMPONENT"},
...
def parse_date_flexible(date_str: str) -> Optional[datetime]:
    ...
    srb_to_eng = {"јан": "Jan", "feb": "Feb", "мар": "Mar", "апр": "Apr", "мај": "May", "јун": "Jun", "јул": "Jul", "авг": "Aug", "сеп": "Sep", "окт": "Oct", "нов": "Nov", "дец": "Dec", "jan": "Jan", "feb": "Feb", "mar": "Mar", "apr": "Apr", "may": "May", "jun": "Jun", "jul": "Jul", "avg": "Aug", "sep": "Sep", "okt": "Oct", "nov": "Nov", "dec": "Dec"}
...
class UniversalCGMParser:
    def __init__(self):
        self.pages, self.words, self.lines, self.regions, self.candidates, self.anchors, self.associations = [], [], [], [], [], [], []
    ...
    def parse(self, pdf_bytes: bytes) -> Dict[str, Any]:
        ...
        actual_components = {
            "VERY_LOW": range_values.get("VERY_LOW"), "LOW": range_values.get("LOW"),
            "IN_RANGE": range_values.get("IN_RANGE"), "HIGH": range_values.get("HIGH"),
            "VERY_HIGH": range_values.get("VERY_HIGH"), "GMI": metric_values.get("GMI"),
            "CV": metric_values.get("CV"), "ACTIVE_TIME": metric_values.get("ACTIVE_TIME"),
        ...
        return {
            "status": status, "reporting_period": reporting_period,
            "actual_components": actual_components, "derived_metrics": derived_metrics,
            "validation_warnings": validation_warnings, "review_reasons": review_reasons
        }
    ...
    def _ingest_pdf(self, pdf_bytes: bytes):
        ...
        for page_number in range(min(2, len(document))):
            ...
            for index, item in enumerate(page.get_text("words")):
                ...
                self.words.append({"page": page_number + 1, "index": index, "bbox": [float(item[0]), float(item[1]), float(item[2]), float(item[3])], "text": item[4].strip(), "normalized": normalize_text(item[4]), "block_no": item[5], "line_no": item[6], "word_no": item[7]})
        document.close()
    ...
    def _build_lines(self):
        ...
        for key, words in grouped.items():
            words = sorted(words, key=lambda w: w["x0"] if "x0" in w else w["bbox"][0])
            lines.append({"line_id": len(lines), "page": key[0], "block_no": key[1], "line_no": key[2], "text": " ".join(w["text"] for w in words), "normalized": normalize_text(" ".join(w["text"] for w in words)), "bbox": bbox_union([w["bbox"] for w in words]), "words": words})
    ...
    def _extract_candidates(self):
        ...
        for line in self.lines:
            ...
            for match in NUMBER_PATTERN.finditer(line["text"]):
                ...
                self.candidates.append({"candidate_id": len(self.candidates), "value": val, "unit": unit, "bbox": line["bbox"], "page": line["page"], "line_id": line["line_id"], "region_id": self._region_for_line(line["line_id"])})
            ...
            for metric, definition in Metrics.items():
                for alias in definition["aliases"]:
                    if alias in line["normalized"]:
                        self.anchors.append({"anchor_id": len(self.anchors), "metric": metric, "page": line["page"], "line_id": line["line_id"], "region_id": self._region_for_line(line["line_id"]), "bbox": line["bbox"]})
                        break
        ...
        for metric in target_metrics:
            ...
            for a in anchors:
                for c in self.candidates:
                    ...
                    if vertical_distance(a["bbox"], c["bbox"]) <= 180:
                        result[metric] = c["value"]
                        break
    ...
    def _review_reasons(self, actual, derived, period): return []
