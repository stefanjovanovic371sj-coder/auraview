...
class UniversalCGMParser:
    ...
    def _extract_actual_ranges(
        ...
        # -----------------------------------------------------
        # Pokušaj 1: Vrednost i label u ISTOJ liniji.
        # -----------------------------------------------------
        for line in lines:
                ...
                for key in list(missing_keys):
                    ...
                    for candidate_line in nearby:
                        if self._is_goal_line(candidate_line):
                            continue
                        matches = re.findall(r"([<>≤≥]?\s*\d+(?:\.\d+)?)\s*%", candidate_line)
                        for raw in matches:
                    ...
                    # OVDJE JE BILA IZMJENA: len(candidates) >= 1 umjesto == 1
                    if len(candidates) >= 1:
    ...
    def _extract_metrics(
        ...
        result["GMI"], warning = self._extract_metric_after_label(
            lines,
            labels=["glucose management indicator", "gmi", "indikator upravljanja"],
            unit="percent",
            key="GMI"
        )
        if warning:
        ...
        result["CV"], warning = self._extract_metric_after_label(
            lines,
            labels=["glucose variability", "coefficient of variation", "variability", "varijabilnost", "koeficijent varijacije", "cv"],
            unit="percent",
            key="CV"
        )
        if warning:
        ...
        result["ACTIVE_TIME"], warning = self._extract_metric_after_label(
            lines,
            labels=["time cgm active", "time cgm", "cgm active", "time sensor active", "sensor active", "active time", "aktivno vreme"],
            unit="percent",
            key="ACTIVE_TIME"
        )
        if warning:
        ...
        result["AVG_GLUCOSE"], warning = self._extract_average_glucose(lines)
        if warning:
            warnings.append(warning)
    ...
    def _extract_metric_after_label(
        ...
        for i, line in enumerate(lines):
            if self._is_goal_line(line):
                continue
            if any(self._contains_label(line, label) for label in labels):
                label_indices.append(i)

        if not label_indices:
            return (None, f"Nedostaje {key}.")

        candidates = []

        for index in label_indices:
            nearby = lines[index:min(len(lines), index + 5)]
            ...
            for j, line in enumerate(nearby):
                ...
                if unit == "percent":
                    ...
                    for raw in matches:
                        value = self._to_float(raw)
                        if value is not None:
                            candidates.append((index + j, value))
        ...
        # OVDJE JE BILA IZMJENA: len(unique_values) >= 1 umjesto == 1
    ...
    def _extract_average_glucose(
        ...
        labels = ["average glucose", "mean glucose", "prosečna glukoza", "prosecna glukoza", "mbg"]
        candidates = []

        for i, line in enumerate(lines):
            if self._is_goal_line(line):
                continue
            if not any(self._contains_label(line, label) for label in labels):
                continue

            nearby = lines[i:min(len(lines), i + 5)]
            ...
            for j, candidate_line in enumerate(nearby):
                ...
                matches = re.findall(
                    r"([<>≤≥]?\s*\d+(?:\.\d+)?)\s*(mmol\s*/?\s*l|mg\s*/?\s*dl)",
                    candidate_line,
                    re.IGNORECASE
                )
                for raw_value, unit in matches:
                    ...
                    if unit_normalized in ("mg/dl", "mgdl"):
                        value = value / 18.0
    ...
    def _is_goal_line(self, line: str) -> bool:
        goal_terms = [
            "goal:", "goal ", "target:", "target ", "cilj:", "cilj ", "reference:", "reference ", "recommended target"
        ]
        return any(term in line for term in goal_terms)

    # =========================================================
    # LABEL MATCH
    # =========================================================

    def _contains_label(self, line: str, label: str) -> bool:
        label = self._normalize_text(label)
        if label == "cv":
            return bool(re.search(r"\bcv\b", line))
        if label == "mbg":
            return bool(re.search(r"\bmbg\b", line))
    ...
    def _extract_reporting_period(
        ...
        for i, line in enumerate(lines):
            if ("reporting period" not in line and "period izveštaja" not in line and "period izvestaja" not in line):
                continue
            ...
            period = self._parse_two_dates(combined)
            if period["start"] and period["end"]:
                return period, None

        dates = []
        for line in lines:
            if ("printing date" in line or "print date" in line or "datum štampanja" in line or "datum stampanja" in line):
                continue
    ...
    def _parse_two_dates(self, text: str) -> Dict[str, Optional[str]]:
    ...
    def _validate_ranges(self, actuals: Dict[str, Any]) -> List[str]:
        warnings = []
        keys = ["VERY_LOW", "LOW", "IN_RANGE", "HIGH", "VERY_HIGH"]
        values = [actuals.get(key) for key in keys]
        ...
        for key in keys:
            ...
            if value < 0 or value > 100:
                warnings.append(f"{key} ima nemoguć procenat: {value}%.")

        if len(present) == 5:
            total = round(sum(present), 2)
            if abs(total - 100) > 2:
                warnings.append(f"Glucose range procenti daju zbir {total}%, očekivano približno 100%.")
    ...
    def _validate_metrics(self, actuals: Dict[str, Any]) -> List[str]:
        warnings = []
        percent_metrics = ["GMI", "CV", "ACTIVE_TIME"]
        ...
        avg = actuals.get("AVG_GLUCOSE")
        if avg is not None:
            if avg <= 0:
                warnings.append("AVG_GLUCOSE mora biti veći od 0.")
    ...
    def _build_review_reasons(
        ...
        reasons = []
        required = ["VERY_LOW", "LOW", "IN_RANGE", "HIGH", "VERY_HIGH", "GMI", "CV", "ACTIVE_TIME", "AVG_GLUCOSE"]

        for key in required:
            if actuals.get(key) is None:
                reasons.append(f"Nedostaje {key}.")

        if not reporting_period.get("start") or not reporting_period.get("end"):
            reasons.append("Nedostaje reporting period.")
    ...
    def _manual_review(
        ...
        result = {
            ...
            "derived_metrics": {"TBR": None, "TIR": None, "TAR": None},
            "validation_warnings": [],
